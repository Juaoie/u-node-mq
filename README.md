...
[点击跳转<===](https://juaoie.github.io/u-node-mq/)

## 可能实现得功能

1 动态提示 queueName 名称功能

## 即将上线的功能

1 创建消费者传递固定的数据给消费方法 `已实现`

2 emit 的方法返回 this，提供链式调用方法 `已实现`
...
6 消息确认提供 next 方法进行手动确认 `已上线`

7 实现真的队列分发消息给消费者模式 `已上线`
