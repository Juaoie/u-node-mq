...
export default class Consumer<D> {
  ...
  consumption(news: News<D>, ask: boolean): Payload<D> {
    const then = (thenParameter: ThenParameter<D>) => {
      ...
      try {
        const confirm: Next = (value) =>
          thenParameter({ isOk: value, consumer: this, news })
        const res = this.consume(
          news.content,
          ask ? confirm : undefined,
          this.payload
        )
        if (ask === false)
          return thenParameter({ isOk: true, consumer: this, news })
